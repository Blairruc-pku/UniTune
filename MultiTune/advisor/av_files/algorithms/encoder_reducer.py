...
sys.path.append(o_path + "MultiTune/advisor/av_files/")

trainset_dir = o_path + "MultiTune/advisor/av_files/trainset/"
enc_rdc_path = o_path + "MultiTune/advisor/av_files/encoder_reducer/"
enc_rdc_res_path = o_path + "MultiTune/advisor/av_files/encoder_reducer/"
if not os.path.exists(enc_rdc_res_path):
    os.makedirs(enc_rdc_res_path)
...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, input_size, gru_input_size, gru_hidden_size, embedding, context_feature=None):
    ...
    def forward(self, input, hidden):
        ...
        if tag == "identifier" or tag == "keyword":
            ...
            output = self.float2bin_tensor(input,
                                           vec_size=self.gru_input_size).view(
                                               1, 1, -1)
        elif tag == "string":
    ...
    def initHidden(self):
        hidden = torch.zeros(1, 1, self.gru_hidden_size, device=device)

        if self.context_feature is not None:
            for i in range(5):
                hidden[0][0][i] = self.context_feature[i]
        return hidden
...
class AttnReducerRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding = embedding
        self.attn = nn.Linear(self.gru_input_size + self.gru_hidden_size,
                              self.gru_hidden_size)
        self.attn_combine = nn.Linear(
            self.gru_input_size + self.gru_hidden_size, self.gru_input_size)
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        if tag == "identifier" or tag == "keyword":
            ...
            # number input=number:float
            embedded = self.float2bin_tensor(
                input, vec_size=self.gru_input_size).view(1, 1, -1)
        elif tag == "string":
            # string input=string:string
            input = self.string2tensor(input).view(1, 1, -1)
            input = F.max_pool1d(F.relu(self.conv1(input)), 2)
...
class Encoder_reducer(object):
    ...
    def __init__(self, context_feature=None):
        self.q_data = dict()
        self.mv_data = dict()
        self.q_mv_data = dict()
        self.triples = []
    ...
    def read_langs(self):
        ...
        # read q
        self.q_data = csv_data_loader(trainset_dir + "data/querydata.csv", [1, 3])

        # read mv
        self.mv_data = csv_data_loader(trainset_dir + "data/mvdata.csv", [1, 3])

        # read q-mv
        self.q_mv_data = csv_data_loader(trainset_dir + "data/query-mvdata.csv",
                                         [3])
    ...
    def split_train_validate(self, validate_size=None):
        ...
        def random_train_vali_index(lis, vali_rate):
            ...
            for i in range(part_num):
                train_index = indexes[:i * vali_num] + indexes[(i + 1) *
                                                               vali_num:]
        ...
        train_vali_idx_lis = random_train_vali_index(self.all_triples,
                                                     self.validate_size)
    ...
    def index_from_word(self, word):
        return self.dictionary.word2index[
            word] if word in self.dictionary.word2index else 1
    ...
    def train(self,
        ...
        for ei in range(q_length):
            ...
            encoder_output, encoder_hidden, tim_output = encoder(
                element, encoder_hidden)
            encoder_outputs[ei] = encoder_output[0, 0]

        q_target_tim = torch.tensor([[q_tim]],
                                    dtype=torch.float,
                                    device=device)
        ...
        for di in range(mv_length):
            ...
            if element[1] == "identifier" or element[1] == "keyword":
                element = (self.index_from_word(element[0]), element[1])
            reducer_output, reducer_hidden, reducer_attention = reducer(
                element, reducer_hidden, encoder_outputs)

        q_mv_target_tim = torch.tensor([[q_mv_tim]],
                                       dtype=torch.float,
                                       device=device)
    ...
    def trainIters(self,
        ...
        reducer_optimizer = optim.Adam(reducer.parameters(),
                                       lr=rdc_learning_rate)
        training_triples = [
            random.choice(self.triples) for i in range(n_iters)
        ]
        criterion = nn.SmoothL1Loss()

        for iter in range(1, n_iters + 1):
            if (BUDGET_FLAG):
                with open(enc_rdc_res_path + "log.txt", "a") as fd:
                    fd.write("budget used up while doing trainIters()\n")
                # print("budget used up while doing trainIters()\n")
                break
            ...
            loss = self.train(q_seq, mv_seq, q_tim, q_mv_tim, encoder, reducer,
                              encoder_optimizer, reducer_optimizer, criterion,
                              self.max_length)
            ...
            if iter % plot_every == 0:
                ...
                if len(self.triples_vali) > 0:
                    ...
                    for triple_vali in self.triples_vali:
                        ...
                        vali_loss += self.validate(q_seq, mv_seq, q_tim,
                                                   q_mv_tim, encoder, reducer,
                                                   criterion)
            ...
            if iter % print_every == 0:
                ...
                with open(enc_rdc_res_path + "log.txt", "a") as fd:
                    fd.write(
                        '%s (%d %d%%) trainLoss: %.4f, valiLoss: %.4f\n' %
                        (self.timeSince(start, iter / n_iters), iter, iter /
                         n_iters * 100, print_loss_avg, last_validate_loss))
                # print('%s (%d %d%%) trainLoss: %.4f, valiLoss: %.4f' %
                #       (self.timeSince(start, iter / n_iters), iter, iter /
                #        n_iters * 100, print_loss_avg, last_validate_loss))
                # sys.stdout.flush()
    ...
    def showPlot(self, points, title):
        ...
        ax.set_title(title)
        plt.plot(points)
        plt.savefig(enc_rdc_res_path + title.replace(" ", "_") + ".png")
    ...
    def evaluate(self,
        ...
        with torch.no_grad():
            ...
            return evl_q_tim, evl_q_mv_tim, reducer_attentions[:mv_length, :
                                                               q_length], reducer_hidden[
                                                                   0, 0]
    ...
    def evaluateRandomly(self, encoder=None, reducer=None, triples=None, n=10):
        ...
        for i in range(n):
            ...
            with open(enc_rdc_res_path + "log.txt", "a") as fd:
                fd.write('> q = {0}, mv = {1}\n'.format(triple[0], triple[1]))
            # print('> q = {0}, mv = {1}'.format(triple[0], triple[1]))
            q_tim = self.scale.detransform(q_tim)
            q_mv_tim = self.scale.detransform(q_mv_tim)
            with open(enc_rdc_res_path + "log.txt", "a") as fd:
                fd.write(
                    '= q_time = {0}, mv_time = {1}, q_mv_time = {2}\n'.format(
                        q_tim, mv_tim, q_mv_tim))
            # print('= q_time = {0}, mv_time = {1}, q_mv_time = {2}'.format(
            #     q_tim, mv_tim, q_mv_tim))
            evl_q_tim, evl_q_mv_tim, attentions, _ = self.evaluate(
                q_seq, mv_seq, encoder, reducer)
            with open(enc_rdc_res_path + "log.txt", "a") as fd:
                fd.write('< q_time = {0}, q_mv_time = {1}\n'.format(
                    evl_q_tim, evl_q_mv_tim))
            # print('< q_time = {0}, q_mv_time = {1}'.format(
            #     evl_q_tim, evl_q_mv_tim))
            # print('')
    ...
    def evaluateAll(self,
        ...
        for triple in triples:
            ...
            q_mv_tim = self.scale.detransform(q_mv_tim)
            with open(enc_rdc_res_path + "log.txt", "a") as fd:
                fd.write('= q_time = {0}, mv_time = {1}, q_mv_time = {2}\n'.format(
                q_tim, mv_tim, q_mv_tim))
            # print('= q_time = {0}, mv_time = {1}, q_mv_time = {2}'.format(
            #     q_tim, mv_tim, q_mv_tim))
            evl_q_tim, evl_q_mv_tim, attentions, _ = self.evaluate(
            ...
            #     evl_q_tim, evl_q_mv_tim))
            res_lis.append(
                [triple[2], q_tim, q_mv_tim, evl_q_tim, evl_q_mv_tim])
        with open(enc_rdc_res_path + "eval_all{0}.csv".format(log_id),
                  "w",
                  newline='') as fp:
    ...
    def showAttention(self, q_id, mv_id, q_seq, mv_seq, attentions):
        ...
        plt.savefig(enc_rdc_res_path + "{0}-{1}.pdf".format(q_id, mv_id))

        # plt.show()
        plt.close('all')

    def evaluateAndShowAttention(self,
                                 q_id,
                                 mv_id,
                                 encoder=None,
                                 reducer=None):
        ...
        evl_q_tim, evl_q_mv_tim, attentions, _ = self.evaluate(
            q_seq, mv_seq, encoder, reducer)
        with open(enc_rdc_res_path + "log.txt", "a") as fd:
            fd.write('input: q = {0}, mv = {1}\n'.format(q_id, mv_id)) 
        # print('input: q = {0}, mv = {1}'.format(q_id, mv_id))
        self.showAttention(q_id, mv_id, q_seq, mv_seq, attentions)

    def save_model(self):
        torch.save(self.encoder.state_dict(), enc_rdc_res_path + "encoder.pth")
        torch.save(self.reducer.state_dict(), enc_rdc_res_path + "reducer.pth")
        with open(enc_rdc_res_path + "dic.pk", "wb") as fp:
            pickle.dump(self.dictionary, fp)
        with open(enc_rdc_res_path + "scale.pk", "wb") as fp:
            pickle.dump(self.scale, fp)

    def load_model(self, enc_rdc_path):
        #with open(enc_rdc_path + "dic.pk", "rb") as fp:
        #    self.dictionary = pickle.load(fp)
        self.prepare_data()
        #with open(enc_rdc_path + "scale.pk", "rb") as fp:
        #    self.scale = pickle.load(fp)

        self.embedding = nn.Embedding(137, self.gru_input_size)

        self.encoder = EncoderRNN(self.dictionary.n_words,
                                  self.gru_input_size,
                                  self.gru_hidden_size,
                                  self.embedding).to(device)
        self.reducer = AttnReducerRNN(self.dictionary.n_words,
                                      self.gru_input_size,
                                      self.gru_hidden_size,
                                      self.embedding,
                                      dropout_p=0.1).to(device)
        #encoder_net = torch.load(enc_rdc_path + "encoder.pth",
         #                         map_location="cpu")
        #self.encoder.load_state_dict(encoder_net)
        self.encoder.to(device)

        #reducer_net = torch.load(enc_rdc_path + "reducer.pth",
        #                         map_location="cpu")
        #self.reducer.load_state_dict(reducer_net)
        ...
        if self.context_feature is not None:
            self.encoder.context_feature = self.context_feature

    def load_pretrain_encoder_model(self, enc_rdc_path):
        self.encoder = torch.load(enc_rdc_path + "encoder.pth",
                                  map_location="cpu")
        self.encoder.to(device)
        self.encoder.train()
        with open(enc_rdc_path + "dic.pk", "rb") as fp:
    ...
    def do_continuously(self, budget=600):
        set_budget_timer(budget)
        ...
        for turn in range(folders):
            if (BUDGET_FLAG):
                with open(enc_rdc_res_path + "log.txt", "a") as fd:
                    fd.write("budget used up while doing do_continuously()\n")
                # print("budget used up while doing do_continuously()\n")
                break
            ...
            self.re_dictionary()
            # self.embedding = nn.Embedding(self.dictionary.n_words,
            #                               self.gru_input_size)
            ...
            self.trainIters(self.encoder,
                            self.reducer,
                            max(int(budget / 2 / part_num), 10),
                            print_every=min(
                                max(int(budget / 2 / part_num), 10), 1000),
                            plot_every=min(max(int(budget / 2 / part_num), 10),
                                           100),
                            log_id=str(turn),
    ...
    def init_do_continuously(self, budget=600):
        self.load_model(enc_rdc_path)
        self.prepare_data()
        self.do_continuously(budget)
        self.save_model()
    ...
    def do(self):
        ...
        for turn in range(folders):
            ...
            self.evaluateAndShowAttention("1a", "mv1", self.encoder,
                                          self.reducer)

            self.evaluateAndShowAttention("1a", "mv2", self.encoder,
                                          self.reducer)

            self.evaluateAndShowAttention("2a", "mv3", self.encoder,
                                          self.reducer)

            self.evaluateAndShowAttention("2a", "mv6", self.encoder,
                                          self.reducer)
    ...
    def do_with_pretrain(self):
        ...
        for turn in range(folders):
            ...
            self.re_dictionary()
            weight = torch.zeros(self.dictionary.n_words,
                                 self.gru_input_size,
                                 device=device,
                                 dtype=torch.float)
...
if __name__ == "__main__":
    budget_lis = [600, 1800, 3600, 7200, 14400]
    tmp_path = enc_rdc_res_path
    for budget in budget_lis:
        enc_rdc_res_path = tmp_path + str(budget) + "/"
        if not os.path.exists(enc_rdc_res_path):
            os.makedirs(enc_rdc_res_path)
        encoder_reducer = Encoder_reducer()
        encoder_reducer.init_do_continuously(budget=budget)
